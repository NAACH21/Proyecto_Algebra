...
if __name__ == '__main__':
    """""# Ejemplo de uso
    # Creamos una matriz de variables predictoras X y un vector y
    X = np.array([[4, 2], [3, 4], [5, 6]])  # Matriz de predictores (debe ser de rango completo)
    y = np.array([1, 2, 3])  # Vector de la variable dependiente
    ...
    try:
        beta = regression_coefficients(X_with_intercept, y)
        mse, r2 = calculate_errors(X_with_intercept, y, beta)
    ...
    print("====================================================")
    # Agregar una columna de unos a X
    X_with_intercept = add_intercept_column(x_2)

    #X_with_intercept = add_intercept_column(X_standardized[:, :3])  # Usamos solo las 3 primeras variables

    try:
        beta = regression_coefficients(X_with_intercept, y_2)
        mse, r2 = calculate_errors(X_with_intercept, y_2, beta)
        print("Coeficientes de regresión (incluyendo intercepto):", beta)
        print("Error cuadrático medio (MSE):", mse)
        print("Coeficiente de determinación (R^2):", r2)
        print_regression_equation(beta, include_intercept=True)
